...
void process_client(SOCKET sock, HCRYPTPROV prov, struct Users users, HCRYPTKEY tgs_key)
{
  ...
  generate_session_key(prov, tgs_key, &session_key_size, session_key_blob);

  printf("Encrypt session key with user key (1st part of the message)\n");
  DWORD msg_len = do_crypt(user_key, ENCODE, session_key_blob, buf, session_key_size, 10240);
  printf("Sending 1st part of the message:\n");
  ...
  printf("Create a mandate (2nd part of the message)\n");
  ...
  printf("Sending 2nd part of the message:\n");
  print_hex_binary(buf, msg_len);
  retVal = send(sock, buf, msg_len, 0);
  if (retVal == SOCKET_ERROR) {
    fprintf(stderr, "Unable to send\n");
...
{
  if (argc != 4) {
    fprintf(stderr, "Usage: %s <users_file> <tgs_key_file> <server_port>", argv[0]);
    return -1;
  }
  ...
  SOCKET sock = initialize_server_socket(argv[3]);
